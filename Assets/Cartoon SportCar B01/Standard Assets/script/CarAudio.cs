...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAudio : MonoBehaviour
    {
        ...
        {
            // get the car controller (this will not be null as we have required component)
        ...
        private void Update()
        {
            // Debug.Log("Update method started.");

            // get the distance to the main camera
            float camDist = (Camera.main?.transform.position - transform.position)?.sqrMagnitude ?? 0;

            // Debug.Log($"Camera distance: {camDist}");
            ...
            if (m_StartedSound)
            {
                ...
                float pitch = ULerp(lowPitchMin, lowPitchMax, m_CarController?.Revs ?? 0);
                ...
                else
                {
                    ...
                    float accFade = Mathf.Abs(m_CarController?.AccelInput ?? 0);
                    float decFade = 1 - accFade;

                    // Debug.Log($"Acceleration Fade: {accFade}, Deceleration Fade: {decFade}");

                    // get the high fade value based on the car's revs
                    float highFade = Mathf.InverseLerp(0.2f, 0.8f, m_CarController?.Revs ?? 0);
                    float lowFade = 1 - highFade;

                    // Debug.Log($"High Fade: {highFade}, Low Fade: {lowFade}");
                    ...
                    // Debug.Log($"Adjusted High Fade: {highFade}, Adjusted Low Fade: {lowFade}, Adjusted Acceleration Fade: {accFade}, Adjusted Deceleration Fade: {decFade}");
